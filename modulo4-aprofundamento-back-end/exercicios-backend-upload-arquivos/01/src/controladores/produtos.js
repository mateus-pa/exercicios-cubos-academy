...
const cadastrarProduto = async (req, res) => {
    ...
    try {
        ...
        const imagem = await storage.uploadFile(
...
const atualizarProduto = async (req, res) => {
    ...
    if (!nome && !estoque && !preco && !categoria && !descricao && !req.file) {
    ...
    try {
        ...
        await storage.deleteFile(produtoEncontrado.imagem);

        const uploadImagem = await storage.uploadFile(
...
const excluirImagemProduto = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json('a imagem do produto foi excluida com sucesso.');
    } catch (error) {
        return res.status(400).json(error.message);
    }
}
