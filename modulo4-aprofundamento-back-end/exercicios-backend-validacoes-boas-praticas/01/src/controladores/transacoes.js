...
const funcoes = require('../utils/funcoesUtilitarias');

const transacoesControlador = {};

transacoesControlador.listar = async (req, res) => {
...
transacoesControlador.detalhar = async (req, res) => {
    const usuario = req.usuario;
    const { id } = req.params;
...
transacoesControlador.cadastrar = async (req, res) => {
    const usuario = req.usuario;
    ...
    try {
        ...
        if ('DateTimeParseError') {
            return res.status(400).json({ mensagem: 'O campo data necessita ser do tipo date' });
        }
...
transacoesControlador.atualizar = async (req, res) => {
    const usuario = req.usuario;
    const { id } = req.params;
    const { descricao, valor, data, categoria_id, tipo } = req.body;
...
transacoesControlador.remover = async (req, res) => {
    const usuario = req.usuario;
    const { id } = req.params;
...
transacoesControlador.obterExtrato = async (req, res) => {
    const usuario = req.usuario;
